...
- `main_window.py::MainWindow`
  - 工具列動作：Load Image、Crop ROI（可勾選）、Clear ROI、Export GC、
    Export JSON。工具列依用途分組（File | Analysis | Export），用分隔線隔開。
    **Estimate Period 不在工具列上**——它是結果欄頂端那顆全寬主按鈕。
  ...
  - **Estimate Period** 是唯一的主要動作，只放在結果欄頂端一顆全寬的橙色主按鈕
    （`btn_estimate`，`variant="primary"`），不放在工具列；忙碌時透過
    `_set_estimate_enabled` 停用。
